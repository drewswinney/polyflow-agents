...
Built for a single operator (the repo owner), not a team. Source of truth for agent behavior: `docs/architecture.md` in `polyflowlabs/polyflow-agents` (branch `docs/initial-architecture`) — §2.4 events, §5 transport, §7 screens.
...
- `github.md` — the source repo association (`polyflowlabs/polyflow-agents`, branch `docs/initial-architecture`) and a screen-to-doc map.
